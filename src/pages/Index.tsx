...
import { ArrowRight, BookOpen, BrainCircuit, BriefcaseBusiness, Code2, ExternalLink, FileText, Github, Linkedin, Mail, Menu, Minus, Moon, Plus, RotateCcw, Smartphone, Sparkles, Sun, UsersRound, X } from "lucide-react";
...
const featuredWork = [
  ...
  { number: "05", name: "Telezi AI", eyebrow: "Creator assistant · iOS", description: "A SwiftUI creator recording assistant that turns an idea into a camera-ready script and helps creators deliver it naturally with an intelligent teleprompter.", role: "Product strategy, SwiftUI development, AI workflow design", tech: ["SwiftUI", "AI", "Teleprompter"], href: "https://tarantulaai.sabsi.sbs/", linkLabel: "Visit product", appStoreHref: "https://apps.apple.com/ph/app/telezi-teleprompter-no-ads/id6804414952", image: "/TeleziAI/logo.png", tone: "from-[#111111] to-[#737373]" },
  { number: "06", name: "Bree", eyebrow: "Wellbeing companion · iOS", description: "A private, offline-first breathing companion that helps people pause, try one small practice, and find a gentler next step.", role: "Product design, SwiftUI development, local-first architecture", tech: ["SwiftUI", "Offline-first", "StoreKit"], href: "http://bree.sabsi.sbs/", linkLabel: "Visit product", appStoreHref: "https://apps.apple.com/ph/app/bree-sleep-meditation/id6806787179", image: "/Bree/logo.png", tone: "from-[#ffd122] to-[#fff1a6]" },
];
...
const experience = [
  { period: "2025", role: "AI Booking Automation Engineer", company: "Freelance / Consulting", image: "/experience/ai-booking-automation.png" },
  { period: "2025—Now", role: "Senior Full Stack AI Software Engineer", company: "Fintelligence" },
  { period: "2024—2025", role: "Senior Fullstack Software Engineer", company: "Teko Philippines" },
  { period: "2020—2024", role: "Full-Stack Backend Software Engineer", company: "Multisys Technologies" },
  { period: "2014—2020", role: "Full-Stack Software Engineer", company: "W-Tech Solutions · Ingram Micro" },
];

const galleryHighlights = [
  { src: "/ShowAndTellMeetup/personal-apps/portfolio-show-and-tell.webp", alt: "Gilbert Tuazon presenting his personal app portfolio" },
  { src: "/ShowAndTellMeetup/personal-apps/telezi-ai-demo.webp", alt: "Telezi AI product demo" },
  { src: "/ShowAndTellMeetup/personal-apps/telezi-ai-teleprompter.webp", alt: "Telezi AI teleprompter workflow" },
  { src: "/ShowAndTellMeetup/personal-apps/telezi-ai-creator-workflow.webp", alt: "Telezi AI creator workflow" },
  { src: "/ShowAndTellMeetup/personal-apps/huawei-visit.webp", alt: "Gilbert Tuazon at the Huawei venue" },
  { src: "/ShowAndTellMeetup/personal-apps/meetup-room.webp", alt: "AppBuildersPH Show and Tell meetup room" },
  { src: "/Fintel/download.webp", alt: "Fintelligence product interface" },
  { src: "/Fintel/download (1).webp", alt: "Fintelligence platform screen" },
  { src: "/Fintel/download (2).webp", alt: "Fintelligence dashboard" },
  { src: "/Fintel/download (3).webp", alt: "Fintelligence product detail" },
  { src: "/Fintel/download (4).webp", alt: "Fintelligence workflow screen" },
  { src: "/Fintel/download (5).webp", alt: "Fintelligence analytics view" },
  { src: "/Fintel/download (6).webp", alt: "Fintelligence platform view" },
  { src: "/Fintel/download (7).webp", alt: "Fintelligence interface detail" },
];

const featuredTikTok = "7646806939032112405";
...
const Index = () => {
  ...
  const galleryTileWidth = Math.round(176 * galleryZoom);
  const galleryTileHeight = Math.round(148 * galleryZoom);
  ...
  return (
    <div className="min-h-screen overflow-hidden bg-background text-foreground">
      ...
      <main id="top">
        ...
        <section id="experience" className="scroll-mt-20 py-20 md:py-32"><div className="mx-auto grid max-w-6xl gap-14 px-5 md:px-8 lg:grid-cols-[0.8fr_1.2fr] lg:gap-24"><Reveal><p className="section-kicker">Experience</p><h2 className="mt-4 text-4xl font-semibold tracking-[-0.04em] md:text-5xl">A decade of shipping, learning, and leading.</h2><p className="mt-6 leading-7 text-muted-foreground">I work best with founders and engineering teams who need someone to clarify the problem, own the hard decisions, and carry the product through delivery.</p><Link to="/cv" className="mt-8 inline-flex items-center gap-2 text-sm font-semibold hover:underline">Full experience and CV <ArrowRight className="h-4 w-4" /></Link></Reveal><Reveal className="border-t border-border">{experience.map(entry => <div key={entry.period} className="grid gap-5 border-b border-border py-6 sm:grid-cols-[110px_1fr]">{entry.image ? <a href={entry.image} target="_blank" rel="noreferrer" className="order-2 overflow-hidden rounded-xl border border-border bg-secondary sm:order-3 sm:col-span-2"><img src={entry.image} alt="n8n AI booking automation workflow" className="h-36 w-full object-cover object-center transition-transform hover:scale-[1.02]" loading="lazy" /></a> : null}<p className="font-mono text-xs text-muted-foreground">{entry.period}</p><div><h3 className="font-semibold">{entry.role}</h3><p className="mt-1 text-sm text-muted-foreground">{entry.company}</p>{entry.image && <p className="mt-2 text-xs leading-5 text-muted-foreground">Conversational booking workflow with availability checks, booking actions, confirmations, and error handling.</p>}</div></div>)}</Reveal></div></section>

        <section className="py-4 md:py-8"><div className="mx-auto max-w-6xl px-5 md:px-8"><Reveal><a href="https://appbuildersph.com/blog/appbuildersph-holds-its-first-ever-show-tell" target="_blank" rel="noreferrer" className="group grid overflow-hidden rounded-3xl border border-border bg-card transition-all hover:-translate-y-1 hover:shadow-xl hover:shadow-black/5 md:grid-cols-[0.9fr_1.1fr]"><div className="min-h-56 overflow-hidden bg-secondary md:min-h-72"><img src="/ShowAndTellMeetup/758793846_4315085988755472_2795391048776258519_n.jpg" alt="AppBuildersPH Show and Tell meetup" className="h-full w-full object-cover transition-transform duration-500 group-hover:scale-105" loading="lazy" /></div><div className="flex flex-col justify-center p-7 md:p-12"><p className="section-kicker">Community highlight</p><h2 className="mt-4 text-3xl font-semibold tracking-[-0.04em] md:text-4xl">AppBuildersPH Show &amp; Tell</h2><p className="mt-4 max-w-xl leading-7 text-muted-foreground">Participated in AppBuildersPH&apos;s first-ever Show &amp; Tell meetup with Filipino app builders, developers, and founders.</p><span className="mt-7 inline-flex items-center gap-2 text-sm font-semibold">Read the event recap <ExternalLink className="h-4 w-4 transition-transform group-hover:translate-x-1" /></span></div></a></Reveal></div></section>

        <section className="py-16 md:py-24"><div className="mx-auto max-w-6xl px-5 md:px-8"><Reveal><p className="section-kicker">Beyond the build</p><h2 className="mt-4 max-w-3xl text-4xl font-semibold tracking-[-0.04em] md:text-6xl">The work, the people, and the things I share along the way.</h2></Reveal><div className="mt-12 grid items-start gap-5 lg:grid-cols-[1.35fr_0.65fr]">
          <Reveal className="min-w-0 self-start rounded-3xl border border-border bg-card p-5 md:p-7"><div className="flex flex-col gap-5 sm:flex-row sm:items-end sm:justify-between"><div><p className="section-kicker">Gallery</p><h3 className="mt-3 text-2xl font-semibold">Projects, demos, and community moments</h3></div><div className="flex flex-wrap items-center gap-3"><div className="flex items-center rounded-full border border-border bg-background p-1" role="group" aria-label="Gallery zoom controls"><button type="button" onClick={() => setGalleryZoom(zoom => Math.max(0.75, Number((zoom - 0.25).toFixed(2))))} disabled={galleryZoom <= 0.75} className="grid h-8 w-8 place-items-center rounded-full transition-colors hover:bg-secondary disabled:cursor-not-allowed disabled:opacity-35" aria-label="Zoom out" title="Zoom out"><Minus className="h-3.5 w-3.5" /></button><span className="min-w-12 text-center font-mono text-[10px] text-muted-foreground">{Math.round(galleryZoom * 100)}%</span><button type="button" onClick={() => setGalleryZoom(zoom => Math.min(1.75, Number((zoom + 0.25).toFixed(2))))} disabled={galleryZoom >= 1.75} className="grid h-8 w-8 place-items-center rounded-full transition-colors hover:bg-secondary disabled:cursor-not-allowed disabled:opacity-35" aria-label="Zoom in" title="Zoom in"><Plus className="h-3.5 w-3.5" /></button><button type="button" onClick={() => setGalleryZoom(1)} className="grid h-8 w-8 place-items-center rounded-full transition-colors hover:bg-secondary" aria-label="Reset gallery zoom" title="Reset zoom"><RotateCcw className="h-3.5 w-3.5" /></button></div><Link to="/gallery" className="hidden items-center gap-2 text-sm font-semibold hover:underline sm:inline-flex">Project archive <ArrowRight className="h-4 w-4" /></Link></div></div><div className="mt-6 min-w-0 overflow-hidden rounded-2xl border border-border bg-secondary/40"><div className="flex items-center justify-between border-b border-border px-4 py-3 text-xs text-muted-foreground"><span>Scroll inside the gallery to explore</span><span className="font-mono">{galleryHighlights.length} images</span></div><div className="max-h-[390px] min-w-0 overflow-auto p-3" role="region" aria-label="Scrollable project and community gallery"><div className="grid w-max grid-flow-col gap-3" style={{ gridAutoColumns: `${galleryTileWidth}px`, gridTemplateRows: `repeat(2, ${galleryTileHeight}px)` }}>{galleryHighlights.map(image => <a key={image.src} href={image.src} target="_blank" rel="noreferrer" className="group relative overflow-hidden rounded-xl bg-secondary" title={`Open ${image.alt}`}><img src={image.src} alt={image.alt} className="h-full w-full object-cover transition-transform duration-500 group-hover:scale-105" loading="lazy" /></a>)}</div></div></div><Link to="/gallery" className="mt-5 inline-flex items-center gap-2 text-sm font-semibold hover:underline sm:hidden">Project archive <ArrowRight className="h-4 w-4" /></Link></Reveal>
          <Reveal className="min-w-0 self-start overflow-hidden rounded-3xl border border-border bg-card"><div className="flex items-center justify-between border-b border-border p-5 md:p-6"><div><p className="section-kicker">TikTok</p><h3 className="mt-2 text-2xl font-semibold">Build notes in motion</h3></div><Link to="/tiktok" className="text-sm font-semibold hover:underline" aria-label="View all TikTok content">View all</Link></div><div className="bg-black"><iframe src={`https://www.tiktok.com/embed/v2/${featuredTikTok}`} className="h-[430px] w-full" style={{ border: "none" }} allowFullScreen allow="encrypted-media" loading="lazy" title="Featured TikTok video" /></div><div className="p-5 md:p-6"><p className="text-sm leading-6 text-muted-foreground">Behind-the-scenes product work, tech experiments, and lessons from shipping.</p><Link to="/tiktok" className="mt-4 inline-flex items-center gap-2 text-sm font-semibold hover:underline">See all content <ArrowRight className="h-4 w-4" /></Link></div></Reveal>
        </div></div></section>
